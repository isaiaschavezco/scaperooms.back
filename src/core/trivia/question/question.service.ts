...
export class QuestionService {
  ...
  async create (createDTO: CreateQuestionDTO): Promise<any> {
    try {
      const questionType = await this.questionTypeRepository.findOne(//Llama el tipo de pregunta (*)
        createDTO.questionType
      )
      let quizz = await this.quizzRepository.findOne(createDTO.quizzId) //Llama al quizz al que corresponde la pregunta que se está creando (*)
  ...
  async update (updateDTO: UpdateQuestionDTO): Promise<any> {
    try {
      ...
      let quizz = await this.quizzRepository.findOne(updateDTO.quizzId) //Llama al quizz al que corresponde la pregunta que se está creando (*)


      // if (quizz.isSend) { Se quitó esta condición por petición, ahora se puede editar aunque ya se haya enviado 18/02 2020 /Isaias
      if (false) {
        response = { status: 7 }
      } else {
